...
class _FSuperState extends State<FSuper> {
  ...
  Widget build(BuildContext context) {
    ...
    FShape fShape = FShape(
      borderShape: widget.cornerStyle == FCornerStyle.round ? FBorderShape.RoundedRectangle : FBorderShape.BeveledRectangle,
      side: borderSide,
    ...
    var decorationImage = widget.backgroundImage != null
        ...
        : null;
    var decoration = decorationImage != null ? ShapeDecoration(image: decorationImage, shape: shape) : null;
    ...
    Widget containerPart = FControl(
      ...
      supportDropShadow: (widget.shadowColor != null && widget.shadowBlur != 0.0) || widget.isSupportNeumorphism,
      dropShadow: FShadow(
        highlightColor: widget.isSupportNeumorphism ? widget.highlightShadowColor ?? Colors.white.withOpacity(0.83) : Colors.transparent,
      ...
      innerShadow: FShadow(
        highlightColor: widget.highlightShadowColor ?? Colors.white.withOpacity(0.83),
      ...
      appearance: widget.isSupportNeumorphism ? FAppearance.Neumorphism : FAppearance.Material,
    ...
    if (widget.maxWidth != null || widget.maxHeight != null) {
      containerPart = LimitedBox(
        ...
        child: Container(width: widget.maxWidth ?? double.infinity, child: containerPart),
      );
    }
  ...
  double get _shadowBlur {
    if ((widget.shadowBlur == null || widget.shadowBlur == 0.0) && widget.isSupportNeumorphism) {
      return 6.0;
    } else {
  ...
  List<Widget> buildChildPart() {
    ...
    if (widget.child1 != null) {
      Offset offset = computeChildOffset(widget.child1Alignment, child1Size, widget.child1Margin);
    ...
    if (widget.child2 != null) {
      Offset offset = computeChildOffset(widget.child2Alignment, child2Size, widget.child2Margin);
      children.add(_MeasureSize(
          onChange: (size) {
            setState(() {
              child2Size = size;
  ...
  Offset computeChildOffset(Alignment? alignment, Size childSize, EdgeInsets? childMargin) {
    ...
    if (alignment == Alignment.topLeft) {
      ...
      childOffset = Offset(size.width - childSize.width, size.height / 2 - childSize.height / 2);
    } else if (alignment == Alignment.bottomRight) {
      childOffset = Offset(size.width - childSize.width, size.height - childSize.height);
    } else if (alignment == Alignment.bottomCenter) {
      childOffset = Offset(size.width / 2 - childSize.width / 2, size.height - childSize.height);
    } else if (alignment == Alignment.bottomLeft) {
      childOffset = Offset(0, size.height - childSize.height);
    } else if (alignment == Alignment.centerLeft) {
      childOffset = Offset(0, size.height / 2 - childSize.height / 2);
    } else {
      childOffset = Offset(size.width / 2 - childSize.width / 2, size.height / 2 - childSize.height / 2);
    }
    if (childMargin != null) {
      childOffset = childOffset.translate(childMargin.left - childMargin.right, childMargin.top - childMargin.bottom);
    }
  ...
  Widget buildRedPoint() {
    bool redPointTextEmpty = widget.redPointText == null || widget.redPointText == "";
    Color redPointTextColor = widget.redPointTextStyle?.color ?? Colors.white;
    double redPointTextSize = redPointTextEmpty ? 0.0 : (widget.redPointTextStyle?.fontSize ?? 0.0);
    var offset = widget.redPointOffset ?? Offset(widget.redPointSize! / 2.0, widget.redPointSize! / 2.0);
    var redPointPart = Positioned(
      right: -offset.dx,
      top: -offset.dy,
      child: Container(
        padding: EdgeInsets.fromLTRB(redPointTextSize / 2.0, 0, redPointTextSize / 2.0, 0),
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.all(Radius.circular(widget.redPointSize! / 2.0)),
        ),
        child: redPointTextEmpty
            ? null
            : Text(
                widget.redPointText ?? "",
                style: widget.redPointTextStyle!.copyWith(color: redPointTextColor, fontSize: redPointTextSize),
              ),
      ),
    );
    return redPointPart;
...
class _RenderStack extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, StackParentData>, RenderBoxContainerDefaultsMixin<RenderBox, StackParentData> {
  ...
  void paint(PaintingContext context, Offset offset) {
    if (_clipBehavior == Clip.hardEdge && _hasVisualOverflow) {
