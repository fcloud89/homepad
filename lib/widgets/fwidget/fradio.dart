  ...
  FRadio({
    ...
    selected = AnimatedContainer(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: (hasSpace || !fill) ? selectedColor : Colors.transparent, width: (hasSpace || !fill) ? border : 0),
    ...
    if (kIsWeb == true) {
      ...
      disableSelected = ColorFiltered(
        colorFilter: ColorFilter.mode(Color(0xfff1f1f1).withOpacity(0.6), BlendMode.srcATop),
        child: selected,
      );
...
class _Radio<T> extends State<FRadio<T>> {
  ...
  _buildChild() {
    if (widget.enable) {
      ...
      return selected ? widget.disableSelected ?? buildDefault() : widget.disableNormal ?? buildDefault();
    }
  }
