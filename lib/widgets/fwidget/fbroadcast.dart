...
class FBroadcast {
  ...
  static FBroadcast instance([dynamic context]) {
    if (context == null) {
      ...
      if (_broadcastMap.containsKey(context) && _broadcastMap[context] != null) {
    ...
    if (fBroadcast != null) {
      ...
      if (reciverInfos1.isEmpty && reciverInfos2.isEmpty) {
        ...
        if (reciverInfos1.isNotEmpty) {
          _fdebugPrint("$tag当前驻留系统的[普通广播]，共 $total1 条：${jsonEncode(reciverInfos1)}");
        }
        if (reciverInfos2.isNotEmpty) {
          _fdebugPrint("$tag当前驻留系统的[Sticky 广播]，共 $total2 条：${jsonEncode(reciverInfos2)}");
        }
      }
    }
  }
...
class _Notifier<T> {
  ...
  void notifyListeners() {
    assert(_debugAssertNotDisposed());
    if (_listeners != null) {
      final List<ResultCallback> localListeners = List<ResultCallback>.from(_listeners!);
