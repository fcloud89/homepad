...
//      cursor: effectiveMouseCursor,
          ...
          if (controlType == FType.Button) {
            ...
            if (widget.controller != null && widget.controller!.states.length > 0) {
              ...
              for (int i = 0; i < widget.controller!.states.length; i++) {
                ...
                if (state != this) {
                  if (state.isSelected == false && state.controlState == FState.Normal) {
                    continue;
                  }
  ...
  Widget createCoreControl() {
    return Container(
      ...
      decoration: ShapeDecoration(
        ...
        shadows: _createDropShadowList(controlState, widget.dropShadow, supportDropShadow),
      ),
      foregroundDecoration: ShapeDecoration(
        shape: _createShapeBorder(controlState, false),
      ),
      child: _createChildContainer(controlState, widget.lightOrientation, widget.innerShadow),
    );
  }

  Widget _createChildContainer(FState? state, FLightOrientation? lightOrientation, FShadow innerShadow) {
    switch (appearance) {
  ...
  void _updateState() {
    if (widget.colorForCallback != null) {
      currentColor = widget.colorForCallback!(widget, controlState) ?? defaultColor;
    ...
    if (widget.gradientForCallback != null) {
      currentGradient = widget.gradientForCallback!(widget, controlState) ?? defaultGradient;
    }

    if (widget.childForStateCallback != null) {
      currentWidget = widget.childForStateCallback!(widget, controlState) ?? defaultWidget;
    }

    if (widget.surfaceForCallback != null) {
      currentSurface = widget.surfaceForCallback!(widget, controlState) ?? defaultSurface;
    }

    if (widget.shapeForStateCallback != null) {
      currentShape = widget.shapeForStateCallback!(widget, controlState) ?? defaultShape;
    }
  }
  ...
  ShapeBorder _createShapeBorder(FState? state, bool justShape) {
    if (widget.shapeForStateCallback != null) {
      currentShape = widget.shapeForStateCallback!(widget, state) ?? defaultShape;
    } else {
      currentShape = defaultShape;
    }
    // justShape = false;
  ...
  List<BoxShadow>? _createDropShadowList(FState? state, FShadow dropShadow, bool canUseShadow) {
    ...
    switch (appearance) {
      ...
      case FAppearance.Material: //Google风格，只处理背光阴影
        ...
        shadows.add(BoxShadow(
          ...
          offset: dropShadow.shadowOffset ?? _dropShadowOffset(appearance, lightOrientation, state, true, dropShadow),
      ...
      default: //FControlAppearance.Neumorphism，新拟态风格，处理向光和背光两个阴影
        ...
        shadows.add(BoxShadow(
          ...
          offset: _dropShadowOffset(appearance, lightOrientation, state, true, dropShadow),
          blurRadius: (state == FState.Highlighted) ? 0 : dropShadow.highlightBlur,
          spreadRadius: (state == FState.Highlighted) ? 0 : dropShadow.highlightSpread,
        ));
        shadows.add(BoxShadow(
          color: dropShadow.shadowColor,
          offset: _dropShadowOffset(appearance, lightOrientation, state, false, dropShadow),
          blurRadius: (state == FState.Highlighted) ? 0 : dropShadow.shadowBlur,
          spreadRadius: (state == FState.Highlighted) ? 0 : dropShadow.shadowSpread,
        ));
    }
    return shadows;
  }
    ...
    switch (lightOrientation) {
      case FLightOrientation.LeftTop:
        if (appearance == FAppearance.Material) {
          ...
          offset = isHighlight ? Offset(-shadow.highlightDistance, -shadow.highlightDistance) : Offset(shadow.shadowDistance, shadow.shadowDistance);
        }
        return offset;
  ...
  Color _innerShadowColor(bool isBacklight, FState? state, FShadow innerShadow) {
  ...
  Offset _innerShadowOffset(bool isBacklight, FState? state, FShadow innerShadow) {
    ...
    switch (lightOrientation) {
      ...
      case FLightOrientation.LeftBottom:
        {
          Offset offset = isBacklight ? Offset(backlightDistance, -backlightDistance) : Offset(-forwardlightDistance, forwardlightDistance);
          if (controlState == FState.Normal || controlState == FState.Disable) {
            offset = Offset.zero;
          }
          return offset;
        }
    ...
    switch (currentSurface) {
      case FSurface.Flat:
        return ShapeDecoration(
            shape: _createShapeBorder(controlState, true), color: (controlState == FState.Highlighted) ? maskColor : Colors.transparent);
          ...
          case FLightOrientation.LeftTop:
            return ShapeDecoration(
              ...
              gradient: LinearGradient(
                  colors: [Colors.transparent, surfaceShadowColor], stops: [0.4, 1.0], begin: Alignment.topLeft, end: Alignment.bottomRight),
            );
          case FLightOrientation.LeftBottom:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
              gradient: LinearGradient(
                  colors: [Colors.transparent, surfaceShadowColor], stops: [0.4, 1.0], begin: Alignment.bottomLeft, end: Alignment.topRight),
            );
          case FLightOrientation.RightTop:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
              gradient: LinearGradient(
                  colors: [Colors.transparent, surfaceShadowColor], stops: [0.4, 1.0], begin: Alignment.topRight, end: Alignment.bottomLeft),
            );
          case FLightOrientation.RightBottom:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
      ...
      case FSurface.Concave:
        switch (lightOrientation) {
          case FLightOrientation.LeftTop:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
              gradient: LinearGradient(colors: [surfaceShadowColor, Colors.transparent], begin: Alignment.topLeft, end: Alignment.bottomRight),
            );
          case FLightOrientation.LeftBottom:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
              gradient: LinearGradient(colors: [surfaceShadowColor, Colors.transparent], begin: Alignment.bottomLeft, end: Alignment.topRight),
            );
          case FLightOrientation.RightTop:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
              gradient: LinearGradient(colors: [surfaceShadowColor, Colors.transparent], begin: Alignment.topRight, end: Alignment.bottomLeft),
            );
          case FLightOrientation.RightBottom:
            return ShapeDecoration(
              shape: _createShapeBorder(controlState, true),
